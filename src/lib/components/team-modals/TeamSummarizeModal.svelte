<!-- ================================================================================
ФАЙЛ: src/lib/components/team-modals/TeamSummarizeModal.svelte
Описание: Модальное окно суммаризации истории для режима "Игра за команду"
================================================================================ -->
...
<script lang="ts">
  ...
  import { summarize } from '$lib/domain/team-chat/teamChat.actions';
  ...
  let isSummarizing = $derived($teamChatStore.isSummarizing);
  const isChatRoute = $derived($page.url.pathname.startsWith('/team/chat/'));
  const chatState = $derived($teamChatStore);
...
<div class="modal-panel summarize-modal">
  <div class="modal-header">
    <h3><i class="fas fa-users"></i> Суммаризация истории команды</h3>
    ...
    {:else}
      <div class="summarize-info">
        ...
        <div class="info-card" style="grid-column: 1 / -1;">
          ...
          <div class="info-value">{chatState.selectedItems?.teamCharacters?.length ?? 0}</div>
          <div class="info-label">Персонажей в команде</div>
        </div>
      </div>
      ...
      <div class="summarize-section">
        ...
        <ul class="summarize-list">
          ...
          <li><i class="fas fa-check"></i> <span>Приватные чаты также перейдут в новую часть</span></li>
        </ul>
      </div>
      ...
      <div class="summarize-section">
        ...
        <textarea
          ...
          rows="4"
          placeholder="Например: Сфокусируйся на развитии отношений между персонажами..."
