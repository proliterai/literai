<!-- ================================================================================
ФАЙЛ: src/lib/components/hero-modals/HeroAnalyticsModal.svelte
Описание: Расширенное модальное окно AI-аналитики для режима "Игра за героя"
================================================================================ -->
<script lang="ts">
	...
	import { analyticsService } from '$lib/domain/hero-analytics/analytics.service'; // исправлен путь
	import type { AnalyticsData, AnalyticsUIState } from '$lib/domain/hero-analytics/analytics.types'; // исправлен путь
	...
	// Derived состояния — исправлен путь на /hero/chat/
	let isChatRoute = $derived($page.url.pathname.startsWith('/hero/chat/'));
	...
	async function runAnalysis(forceUpdate: boolean = false): Promise<void> {
		...
		try {
			...
			console.error('[HeroAnalyticsModal] Ошибка анализа:', e);
	...
	<div class="modal-body">
		...
		{:else if analyticsData}
			<div class="analytics-results">
				...
				<!-- Контент вкладок (оставлен без изменений, все секции будут отображаться,
				     если соответствующие данные присутствуют в analyticsData) -->
