<!-- ================================================================================
ФАЙЛ: src/lib/components/hero-modals/HeroExportModal.svelte
Описание: Модальное окно экспорта для режима "Игра за героя" с поддержкой шифрования
================================================================================ -->
<script lang="ts">
  ...
  function getExportPayload() {
    const chatData = buildHeroSessionRow();
    return {
      type: 'chat_export',
      mode: 'hero',
      version: '2.0',
  ...
  function getBaseFilename(chatData: any) {
    const heroName = sanitizeFilename(chatData.selectedItems?.heroCharacter?.name || 'hero');
    const sceneName = sanitizeFilename(chatData.selectedItems?.scene?.name || 'scene');
    const dateStr = new Date().toISOString().slice(0, 10);
    return `literai_heroplay_${heroName}_${sceneName}_${dateStr}`;
  }
  ...
  async function exportChat() {
    if (!isHeroChatRoute || !validateBeforeExport()) return;
    ...
    try {
      ...
      exportStatus = { type: 'success', message: 'Чат героя успешно экспортирован!' };
    } catch (e: any) {
  ...
  async function handleImageSelect(e: Event) {
    ...
    try {
      ...
      exportStatus = { type: 'success', message: 'История успешно вшита в картинку!' };
    } catch (err: any) {
...
<div class="modal-box export-modal">
  ...
  <h2><i class="fas fa-gamepad"></i> Экспорт истории героя</h2>
  ...
  <div class="export-section">
    ...
    <div class="export-buttons">
      <button class="export-btn export-btn-primary" onclick={exportChat} disabled={exporting || !isHeroChatRoute}>
        {#if exporting}
  ...
  <div class="export-section">
    ...
    <div class="export-buttons">
      <button class="export-btn export-btn-primary" onclick={triggerImageSelect} disabled={exporting || !isHeroChatRoute}>
        {#if exporting}
          <i class="fas fa-spinner fa-spin"></i> Обработка...
        {:else}
