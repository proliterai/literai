<!--
  ФАЙЛ: src/lib/components/hero-modals/HeroScriptModal.svelte
  Описание: Модальное окно скрипта для режима "Игра за героя".
...
<script lang="ts">
  ...
  // --- Системный промпт (герой) ---
  let promptText = $state('');
  let sp = $derived($heroSystemPromptStore);
  ...
  // Определение, находимся ли мы на странице чата героя
  const isHeroChatRoute = $derived($page.url.pathname.startsWith('/hero/chat/'));

  // Состояние чата (для загрузки персонажа)
  let chatState = $derived($heroChatStore);

  // Состояние для выпадающих меню
  let promptSelectorOpen = $state(false);
  let cheatSelectorOpen = $state(false);

  // Синхронизация promptText при смене активного пресета системного промпта
  $effect(() => {
  ...
  onMount(async () => {
    if (!sp.loaded) {
      await heroSystemPromptStore.loadPresets();
    }

    await cheatmodeStore.loadPresets('hero');

    if (!lb.ready) await lorebookStore.init();

    if (isHeroChatRoute && chatState.selectedItems?.heroCharacter) {
      cheatmodeStore.loadCharactersFromSelections({
        heroCharacter: chatState.selectedItems.heroCharacter
      });
    }
  ...
  function handleSetActivePreset(id: string) {
    heroSystemPromptStore.setActivePreset(id);
    promptSelectorOpen = false;
  }

  function handleSaveCustomPrompt() {
    heroSystemPromptStore.saveCustom(promptText);
  }

  function handleResetPrompt() {
    heroSystemPromptStore.resetToDefault();
  }
  ...
  function handleSaveAsNewPreset() {
    ...
    heroSystemPromptStore.saveAsCustomPreset(newPromptName, promptText);
    newPromptName = '';
  ...
  function handleDeletePromptPreset() {
    if (confirm('Удалить этот пресет?')) {
      heroSystemPromptStore.deleteCustomPreset(sp.activePresetId);
    }
  }

  function handleExportPrompt() {
    const data = {
      type: 'hero_prompt_preset',
      name: activeSystemPreset?.name || 'Пресет героя',
      content: promptText
    };
    downloadJson(data, `hero_prompt_${Date.now()}.json`);
  }
  ...
  function handleImportPrompt(e: Event) {
    ...
    file.text().then(text => {
      try {
        heroSystemPromptStore.importPreset(JSON.parse(text));
      } catch (err) {
  ...
  function handleCreateRelationshipAdvanced() {
    ...
    // Очищаем только цель, чтобы было удобно создавать связи от одного источника
    relTargetName = '';
  }
...
<div class="modal-panel script-modal">
  <div class="modal-header">
    <h3><i class="fas fa-dragon"></i> Скрипт (Герой)</h3>
  ...
  <div class="tab-nav">
    ...
    <button class="tab-btn {tab === 'cheatmode' ? 'active' : ''}" onclick={() => {
      tab = 'cheatmode';
      // Используем cm.runtime.actors вместо cm.runtime.characters
      if (isHeroChatRoute && (!cm.runtime.actors || cm.runtime.actors.length < 1) && chatState.selectedItems?.heroCharacter) {
        cheatmodeStore.loadCharactersFromSelections({
          heroCharacter: chatState.selectedItems.heroCharacter
        });
      }
  ...
  <div class="tab-content-area">
        ...
        <div class="preset-selector-wrapper">
          ...
          {#if promptSelectorOpen}
            <div class="preset-selector-dropdown open">
              <!-- Пользовательские пресеты СВЕРХУ -->
    ...
    {:else if tab === 'cheatmode'}
      <div class="cheatmode-layout">
        ...
        {:else}
          <!-- Выпадающее меню пресетов (системные + пользовательские) -->
          <div class="preset-selector-wrapper">
            <button
        ...
        <div class="cheatmode-section" style="margin-top:16px">
          <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom: 12px;">
            <h4 style="margin:0;"><i class="fas fa-heart"></i> Отношения (Персонажи и NPC)</h4>
            <!-- Новые кнопки Out и In -->
          ...
          {:else}
            ...
            <div class="relationship-creator">
              ...
              <div class="modal-field">
                ...
                <select id="rel-preset" class="panel-select" bind:value={selectedRelPresetId}>
