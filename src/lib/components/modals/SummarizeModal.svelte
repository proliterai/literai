<!-- ================================================================================
ФАЙЛ: src/lib/components/modals/SummarizeModal.svelte
Описание: Модальное окно суммаризации истории для ролевого чата
================================================================================ -->
<script lang="ts">
import { get } from 'svelte/store';
import { ui } from '$lib/ui/ui.store';
import { chatStore } from '$lib/domain/chat/chat.store';
import { summarize } from '$lib/domain/chat/chat.actions';
...
// Инициализируем локальный стейт из глобальных настроек
let customPrompt = $state(get(settingsStore).values[SETTINGS_KEYS.SUMMARIZE_PROMPT] || '');
let isSummarizing = $derived($chatStore.isSummarizing);
const isChatRoute = $derived($page.url.pathname.startsWith('/roleplay/chat/'));
...
async function handleSummarize() {
  if (!isChatRoute) {
    ui.notify('Откройте чат для суммаризации', 'warning');
    return;
  }
  
  // 1. Сохраняем измененный промпт в глобальные настройки
  await settingsStore.set(SETTINGS_KEYS.SUMMARIZE_PROMPT, customPrompt);
  
  // 2. Запускаем суммаризацию
  const promptText = customPrompt.trim() || undefined;
  const ok = await summarize(promptText);
  
  // 3. Закрываем только при успехе (экшен вернет true/false)
  if (ok) {
    ui.closeModal();
...
<div class="modal-panel summarize-modal">
  <div class="modal-header">
    <h3><i class="fas fa-book-open"></i> Суммаризация истории</h3>
  ...
  <div class="tab-content-area">
    ...
    <div class="panel-placeholder">
      ...
      <p>Откройте чат для суммаризации истории</p>
    </div>
    ...
    <div class="summarize-section">
      ...
      <ul class="summarize-list">
        <li><i class="fas fa-check"></i> <span>Текущая часть будет завершена и сохранена</span></li>
        <li><i class="fas fa-check"></i> <span>ИИ создаст краткий пересказ ключевых событий</span></li>
        <li><i class="fas fa-check"></i> <span>Начнётся новая часть с обновлённым контекстом</span></li>
        <li><i class="fas fa-check"></i> <span>Предыдущие суммаризации сохранятся для контекста</span></li>
      </ul>
    </div>
    ...
    <div class="summarize-section">
      ...
      <textarea
        ...
        rows="4"
        placeholder="Например: Сфокусируйся на развитии отношений персонажей..."
    ...
    <div class="summarize-actions">
      ...
      <button class="btn-primary" onclick={handleSummarize} disabled={isSummarizing}>
        {#if isSummarizing}
        <i class="fas fa-spinner fa-spin"></i> Суммаризация...
        {:else}
        <i class="fas fa-magic"></i> Создать новую часть
        {/if}
      </button>
    </div>
    {/if}
