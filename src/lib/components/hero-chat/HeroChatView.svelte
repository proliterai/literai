<!-- ================================================================================
ФАЙЛ: src/lib/components/hero-chat/HeroChatView.svelte
Описание: Основной компонент чата для режима "Игра за героя" с сообщениями,
          редактированием, ветвлением, рероллом и суммаризацией.
================================================================================ -->
<script lang="ts">
...
import {
	...
	summarize
} from '$lib/domain/hero-chat/heroChat.actions';
import type { ChatMessage } from '$lib/domain/hero-chat/heroChat.types';
...
let selected = $derived(s.selectedItems); // HeroSelectedItems
...
async function saveEdit(m: ChatMessage) {
	try {
		...
		// Редактирование сообщения ассистента (рассказчика)
		if (mode === 'edit') {
			heroChatStore.updateMessage(id, text);
			ui.notify('Сообщение обновлено', 'success');
		} else {
			heroChatStore.createBranchFromEdit(id, text);
			ui.notify('Создана новая ветвь', 'success');
		}
	} catch (error: any) {
		console.error('[HeroChatView] saveEdit error:', error);
...
function switchVersion(messageId: string, dir: -1 | 1) {
	try {
		heroChatStore.switchVersion(messageId, dir);
	} catch (error: any) {
		console.error('[HeroChatView] switchVersion error:', error);
...
async function deleteMessage(messageId: string) {
	if (await ui.confirm('Подтверждение', 'Удалить это сообщение?')) {
		try {
			heroChatStore.deleteMessageWithPairRule(messageId);
			ui.notify('Сообщение удалено', 'success');
		} catch (error: any) {
			console.error('[HeroChatView] deleteMessage error:', error);
...
function switchBranch(idx: number) {
	try {
		heroChatStore.switchBranch(idx);
	} catch (error: any) {
		console.error('[HeroChatView] switchBranch error:', error);
...
async function deleteBranch(idx: number) {
	...
	if (await ui.confirm('Удаление ветви', `Удалить ветвь "${branchName}"?`)) {
		try {
			heroChatStore.deleteBranch(idx);
			ui.notify('Ветвь удалена', 'success');
		} catch (error: any) {
			console.error('[HeroChatView] deleteBranch error:', error);
...
function switchPart(idx: number) {
	try {
		heroChatStore.switchPart(idx);
	} catch (error: any) {
		console.error('[HeroChatView] switchPart error:', error);
...
async function deletePart(idx: number) {
	...
	if (await ui.confirm('Удаление части', `Удалить часть "${partName}"? Все ветки этой части будут удалены.`)) {
		try {
			heroChatStore.deletePart(idx);
			ui.notify('Часть удалена', 'success');
		} catch (error: any) {
			console.error('[HeroChatView] deletePart error:', error);
...
function updatePreviousPartSummary(value: string) {
	try {
		heroChatStore.updatePreviousPartSummary(value);
	} catch (error: any) {
		console.error('[HeroChatView] updatePreviousPartSummary error:', error);
	}
}
</script>
...
<div class="chat-container">
	...
	>
		{#each visibleMessages as m (m.id)}
			...
			{@const avatarUrl = safeUrl(isUser ? selected?.heroCharacter?.avatar : '')}
			{@const characterName = isUser ? (selected?.heroCharacter?.name ?? 'Герой') : 'Рассказчик'}
			...
			>
								...
								<div class="message-header-controls">
									{#if (m.versions?.length ?? 0) > 1}
										<div class="version-nav">
											...
											<span style="color:white;">{(m.activeVersion ?? 0) + 1}/{m.versions!.length}</span>
				...
				{:else}
					<!-- Режим МАЛЕНЬКАЯ или КРУПНАЯ аватарка -->
					<div class="message-header avatar-style-{avatarStyle}">
						...
						{:else}
							<div class="message-avatar-placeholder">
								<i class="fas fa-feather"></i> <!-- иконка рассказчика -->
							</div>
						{/if}
							...
							<div class="message-header-controls">
								{#if (m.versions?.length ?? 0) > 1}
									<div class="version-nav">
										<button
											class="v-btn"
											disabled={(m.activeVersion ?? 0) === 0}
											onclick={() => switchVersion(m.id, -1)}
										...
										<button
											class="v-btn"
											disabled={(m.activeVersion ?? 0) === m.versions!.length - 1}
											onclick={() => switchVersion(m.id, 1)}
								...
								<button
									...
									disabled={s.isGenerating || s.isRerolling || s.isSummarizing}
