...
# Автоматически переходим в папку, где физически лежит этот скрипт.
# Это гарантирует корректную работу при двойном клике мышкой в Linux/Mac.
cd "$(dirname "$0")"

# 1. Проверка, что мы в корне проекта
if [ ! -f "package.json" ]; then
    echo "[ОШИБКА] Не найден файл package.json."
    echo "Убедитесь, что скрипт находится в корневой папке проекта."
    exit 1
fi
...
# 3. Проверка версии Node.js (>= 18) с поддержкой Mac и Linux
...
# Открываем браузер в фоновом процессе (поддерживает и Linux, и macOS)
